@if (!($_SESSION['user'] ?? false))
    <div class="!p-6 !px-2.5 bg-white fixed bottom-0 z-50 w-full shadow-lg rounded-t-xl" id="bottom-cta">
        <div class="container-cta relative">
            {{-- Close button --}}
            <button class="absolute -top-2 right-1 text-gray-500 hover:text-gray-700" onclick="closeBottomCTA()">
                <ion-icon name="close-circle" class="text-[30px]"></ion-icon>
            </button>
            {{-- Content --}}
            <div class="max-w-[775px] mx-auto flex items-center">
                <img src="/assets/images/logo.png" alt="CYO Logo" class="w-28 h-28 hidden sm:block mr-4">
